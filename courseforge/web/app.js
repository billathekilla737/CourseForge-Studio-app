/* CourseForge Studio - local instructor UI. Talks only to the Python server on 127.0.0.1. */
'use strict';
...
/* Escape gives you the roster back after a range select. */
document.addEventListener('keydown', ev => {
  if (ev.key !== 'Escape') return;
  if ($('#modalHost').innerHTML) return;          // a dialog owns Escape first
  if (S.picked.size) { ev.preventDefault(); clearPicked(); }
});
...
function route() {
  ...
  if (parts[0] === 'c' && parts[2] === 'a')
    return openAssignment(parts[1], parts[3], { arriving: true });
  if (parts[0] === 'c') return openCourse(parts[1]);
  return openCourses();
}
window.addEventListener('hashchange', route);

function showView(which) {
  $('#viewPicker').classList.toggle('hidden', which !== 'picker');
  $('#viewWork').classList.toggle('hidden', which !== 'work');
  $('#viewSchedule').classList.toggle('hidden', which !== 'schedule');
  if (which !== 'work') stopAutoPull();
...
async function openCourse(courseId, refresh) {
  ...
  crumbs([{ label: 'Courses', href: '#/' }, { label: S.course.name }]);
  $('#headerActions').innerHTML = '';
