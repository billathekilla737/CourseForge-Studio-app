...
- `references/slo-alignment.md` — **SLO alignment**: answer "is my Canvas shell aligned to the Student Learning Outcomes for my program?" Holds the MCCB curriculum-framework index (searchable by CIP code and by course PREFIX, so a `course_code` like `IMT 1213 001` resolves to its owning program), a machine-readable program -> framework-PDF table, how to extract the SLOs (locally with PyMuPDF; a web-to-markdown fetch mangles these PDFs), the cross-check to run (each lettered sub-outcome -> the Canvas items that assess it, flagging unassessed SLOs, ungraded-only coverage, unaligned items, and verb-level mismatches where an SLO says "create/implement/compile" but the only evidence is prose), and the traps (local course numbers drift from state numbers, one PDF served under two CIP codes, stale deep links, transition-semester framework versions, and academic-transfer courses that these CTE frameworks do not govern). Use whenever asked whether a course or program is aligned, or to produce an alignment matrix for program review.
