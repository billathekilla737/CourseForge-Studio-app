...
- `scripts/Remediate-CanvasPdfText.ps1` + `scripts/pdf_text_tool.py` — **PDF text SCAN + targeted in-place EDIT** (needs `pip install PyMuPDF`): same three-phase gateway as PPTX/DOCX — `-Action List` enumerates course PDFs, `-Action Fetch -Pattern <regex>` downloads (keeps `original.pdf`) and scans visible text + metadata per page, agent reviews `scan.json` and writes `map.json` (`[{find, replace}]` literals), `-Action Push` applies via redaction-box replacement (white-fill + redraw, layout never reflows), **verifies zero residual "find" strings (refuses upload otherwise)**, then uploads over the same filename/folder (`on_duplicate=overwrite`, links keep working; dry-run default, `-Apply` to upload). Can also set PDF metadata (`--set-author/--set-title`). HONEST SCOPE: short factual strings (names, emails, phones, room numbers) drawn in a base font — replacement is visibly Helvetica next to embedded fonts, a much-shorter replacement leaves a whitespace gap, and it does NOT create tagged/accessible PDFs (that stays triage_pdf.py + Acrobat/regenerate-from-docx). **After apply, RENDER the changed pages to PNG and view them** (`pymupdf get_pixmap`) — text-extraction verify proves the strings, only eyes prove the layout.
